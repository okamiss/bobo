...
4. 添加照片与短视频，等待进度和校验完成；调整顺序、说明和封面。写不出来时可以点正文工具栏右侧的「AI 帮我写」：它会看这篇已上传的前 4 张照片、日期和你填的重点，给出 3 个标题、一段正文、几个标签和每张照片的说明。生成的内容需要你逐项点击应用，再手动保存，AI 不会自动改动任何记录。首次使用会说明哪些内容会发送出去，同意一次即可，同意状态按账号单独保存。每个账号每天最多 20 次；没有配置 AI 服务时这个按钮会提示未配置，网站其余功能不受影响。
...
不配置也能正常使用网站，只是编辑页的「AI 帮我写」会提示未配置。要启用，在 `.env` 中填写密钥后重新部署：
...
发送给模型的内容：这篇记录的日期与类型、你填写的重点，以及选中照片的 640px 缩略图（最多 4 张，由服务端读取后内嵌发送，不会暴露 OSS 地址）。不发送账号、密码、会话和整个数据库。每个账号首次使用会看到这段说明并需要同意一次，同意状态按账号单独保存。

额度与失败处理：每个账号每天最多 20 次（按上海时区计算），用量、耗时和 token 数记在 `AiUsage` 表。模型超时、限流、余额不足或返回的内容不可用时，都会给出对应的中文提示并可重试，不影响已经写好的内容；生成结果必须逐项点击应用，再手动保存，AI 不会自动修改或发布任何记录。
