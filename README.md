...
6. 创建相册，从已经上传的媒体中挑选、排序。删除相册不删除原故事或文件。
   标签输错时，家庭管理员可通过后台「管理标签」或编辑页标签框下的同名按钮，搜索并删除已保存的标签。界面显示使用篇数，确认后从所有记录（含草稿和仅家人可见的记录）中移除同名标签，正文和照片保留，删除操作会写入操作记录；普通家庭成员仍可在自己的记录中添加或移除标签。
