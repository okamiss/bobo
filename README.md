...
聊天记录按账号分开：家人之间看不到彼此聊过什么，可以新建、切换、删除单条或清空全部。切换页面不会中断当前对话。每个账号每天最多 50 句。
...
额度与失败处理：每个账号每天最多 20 次写作、50 句聊天（按上海时区计算），用量、耗时和 token 数记在 `AiUsage` 表。模型超时、限流、余额不足或返回的内容不可用时，都会给出对应的中文提示并可重试，不影响已经写好的内容；生成结果必须逐项点击应用，再手动保存，AI 不会自动修改或发布任何记录。
