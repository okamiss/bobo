...
const ImageLightbox = lazy(() => import("./ImagePreview"));
...
export function Lightbox({
  ...
  if (items[start].kind === "image")
    return (
      <Suspense
        fallback={
          <div className={s.lightbox} role="status">
            正在打开图片…
          </div>
        }
      >
        <ImageLightbox items={items} start={start} onClose={onClose} />
      </Suspense>
    );
  ...
  return (
    <VideoLightbox
      ...
      start={Math.max(
        0,
        videos.findIndex((item) => item.id === items[start].id),
      )}
      onClose={onClose}
    />
  );
