...
- **日期**：`occurredOn`、生日等都是 `YYYY-MM-DD` 字符串而不是 DateTime；「今天」按 Asia/Shanghai 计算。排序是 `occurredOn desc` → `publishedAt desc（null 排最后）` → `createdAt desc` → `id desc`，统一用 main.ts 的 `entryOrder` 常量（retriever.ts 有一份同样的，供聊天检索用）。同一天可能发好几篇，之前拿随机 uuid 当次序等于乱序；`publishedAt` 在首次发布时写入、改回草稿也保留，从未发布的草稿落回 `createdAt`。**改排序必须同步改 `Content.get()` 里的上一篇/下一篇游标**（三段 OR 比较 occurredOn / publishedAt / id），否则翻页会跳记录，`npm run test:ordering` 专门守这条。
...
- **AI 写作助手**：`ai.ts` 的 `AiService`，编辑页「AI 帮我写」用。**可选功能**：`config().ai` 在没有 `DEEPSEEK_API_KEY` 时返回 `null`，接口报「还没有配置」而整站照常运行——服务器的 `.env` 不会被 `update-server.sh` 改写，所以新增的 AI 配置项一律要可选，不能写成必需项。默认模型 `deepseek-flash`（支持图片输入、Tool Calls 和 `json_object`），`AI_MODEL` / `AI_BASE_URL` 可覆盖，后者也是测试打桩的钩子。接口 `/api/admin/ai/status`、`/consent`、`/draft` 所有家人可用，同意状态存在 `Admin.aiConsentAt`。照片由前端只传 `mediaId`、服务端读 `<key>.thumb` 转 base64（绕开 1mb 的 JSON 上限，一次最多 4 张）。`json_object` 没有 schema 强制且官方承认可能返回空内容，所以用 `validation.ts` 的 `aiDraftOutput` 校验并自动重试一次，仍失败才报错；出站调用一律 `AbortSignal.timeout(90s)`（Nginx `proxy_read_timeout` 是 180s），失败按类型抛 `HttpException` 子类，否则全局过滤器会统一变成「服务暂时不可用」。配额记在 `AiUsage` 表、按上海时区当天计数，不能沿用 `failedAttempts`（那个按 IP 且只统计失败）。额度本身存在 `Profile.aiDraftQuota` / `aiChatQuota`（默认 20 / 50），由 owner 通过 `GET|PUT /api/admin/ai/quotas` 调整并写操作记录，成员连读都是 403；改完立即生效，0 表示关闭该功能。`AiService.quotas()` 每次取一次，别把额度写回常量。生成结果只回传给编辑器，应用时走 `change()`，不自动保存。
...
- **新建记录**：`/admin/entries/new` 只在浏览器里放一张空表单，`POST /api/admin/entries` 要到第一次保存或第一次加照片时才发（`EntryEditor.ensureEntry()`），所以打开页面不再留下空记录；创建接口接受 `entryInput.partial()` 但**把 title 覆盖成允许空串**（`.partial()` 只是让键可选，前端传 `title: ""` 时 `.min(1)` 照样会报错，加照片起头的记录就是没名字），保存用的 PUT 仍然必填；`ensureEntry()` 故意不发 `status`/`visibility`，创建一律落成私密草稿，**加照片不能把没写完的记录发到前台**，真正的状态由随后的保存决定。创建完成后用 `replace` 把地址换成真实 id。
- **编辑器的状态分两套，别混用**（`EntryEditor`，这块反复出过 bug）：
  - **渲染和 effect 用 state**：`entryId`、`form`。有依赖数组、点击时取的是当前渲染值，都正确。
  - **异步回调用 ref**：`idRef`（记录 id）、`formRef`（当前内容）。上传和保存的回调是在更早的渲染里建的闭包，中途 `setState` 到不了它们——用 state 会让 `refreshMedia` 请求 `/admin/entries/`（落到列表接口、没有 media 字段，编辑页白屏），也会让每张照片各建一条记录。`ensureEntry` 里三者一起更新：`idRef`（给正在跑的回调）、`setEntryId`（给界面）、`loaded`（防止随后的 GET 覆盖已输入内容）。
  - 判断逻辑尽量放进 `lib.ts` 的纯函数，因为组件交互没法在这里自动测：`mergeMedia`（上传后合并，保本地顺序与未保存的说明）、`withDraft`（恢复草稿，丢弃已删除的照片、封面退回）、`afterSave`（保存往返期间继续输入的内容不丢）、`draftOf`、`moveToFront`，都有单元测试。
  - `AiDraftModal` 只在打开时挂载：它在组件体里就读 `entry.media`，常挂着的话一旦内容异常会连累整个编辑页。
