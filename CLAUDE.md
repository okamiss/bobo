...
- **新建记录**：`/admin/entries/new` 只在浏览器里放一张空表单，`POST /api/admin/entries` 要到第一次保存或第一次加照片时才发（`EntryEditor.ensureEntry()`），所以打开页面不再留下空记录；创建接口接受 `entryInput.partial()` 但**把 title 覆盖成允许空串**（`.partial()` 只是让键可选，前端传 `title: ""` 时 `.min(1)` 照样会报错，加照片起头的记录就是没名字），保存用的 PUT 仍然必填；`ensureEntry()` 故意不发 `status`/`visibility`，创建一律落成私密草稿，**加照片不能把没写完的记录发到前台**，真正的状态由随后的保存决定。创建完成后用 `replace` 把地址换成真实 id，`loaded` ref 保证随后的 GET 不会覆盖本地内容。**新记录的 id 必须用 `idRef` 而不是 `entryId` state**：上传回调是在记录还不存在时创建的闭包，中途 `setEntryId` 到不了它们——用 state 会让 `refreshMedia` 请求 `/admin/entries/`（落到列表接口、没有 media 字段，编辑页白屏），也会让每张照片各建一条记录。`ensureEntry` 同时用 `creating` ref 共享首次创建的 Promise，多张照片只建一条。
